...
def preformat_SyntheticWL(dataset_dir, name):
    ...
    if name.lower() == "trix":
        pre_transform_in_memory(dataset, T.Constant(cat=False))
        pre_transform_in_memory(dataset.test, T.Constant(cat=False))
    return dataset
...
def get_unique_mol_graphs_via_smiles(
    ...
    if cfg.dataset.extra_graphs:
        for reg in range(cfg.dataset.extra_graphs_low, cfg.dataset.extra_graphs_high):
            for _ in range(cfg.dataset.extra_graphs_num):
